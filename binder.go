...
/**
    ...
    * QueryParamsBinder(r) - binds query parameters (source URL)
    * PathParamsBinder(r) - binds path parameters (source URL)
    * FormFieldBinder(r) - binds form fields (body)

	Example:
  ```go
  var length int64
  err := binding.QueryParamsBinder(r).Int64("length", &length).BindError()
  ```
...
func (be *BindingError) Error() string {
	return fmt.Sprintf("%s, field=%s", be.Message, be.Field)
}
...
func ChiPathParamsBinder(r *http.Request) *ValueBinder {
	return &ValueBinder{
		failFast: true,
		ValueFunc: func(sourceParam string) string {
			return chi.URLParam(r, sourceParam)
		},
		ValuesFunc: func(sourceParam string) []string {
			// path parameter should not have multiple values so getting values does not make sense but lets not error out here
			value := chi.URLParam(r, sourceParam)
			if value == "" {
				return nil
			}
			return []string{value}
...
func FormFieldBinder(r *http.Request) *ValueBinder {
	vb := &ValueBinder{
		failFast: true,
		ValueFunc: func(sourceParam string) string {
			return r.PostFormValue(sourceParam)
		},
		ErrorFunc: NewBindingError,
	}
	vb.ValuesFunc = func(sourceParam string) []string {
		...
		values, ok := r.PostForm[sourceParam]
		if !ok {
			return nil
		}
		return values
